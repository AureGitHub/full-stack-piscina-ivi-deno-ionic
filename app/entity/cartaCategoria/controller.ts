...
const entity =new aureDB(client,entities,'cartaCategoria' );


const get= async (ctx: any) => {

    const sqlSelect = ` select *, titulo || ' (' || orden || ')' "tituloPres", case when incluir then 'Sí' else 'No' end "incluirPres" `;

  let sqlFrom =` from "cartaCategoria" c `;


  const orderBydefect = ` order by orden`;
